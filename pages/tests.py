...
class GetCurrentContentAPITests(APITestCase):
    ...
    def test_main_page_includes_seeded_order_confidence_component(self):
        ...
        self.assertEqual(
            component_payload["data"]["subtitle"],
            "ონლაინ ყიდვისას ყველაზე ხშირად რაც აჩენს ეჭვს, აქ წინასწარ ნათელია: პროცესი, რეგისტრაცია, გადახდა და მიწოდება.",
        )
    ...
    def test_terms_page_includes_seeded_component(self):
        ...
        self.assertEqual(
            component_payload["data"]["subtitle"],
            "ეს გვერდი აღწერს FlexDrive-ზე საიტის გამოყენების, ანგარიშის, ავტონაწილების შერჩევის, შეკვეთის, გადახდის, მიწოდებისა და დაბრუნების ძირითად პირობებს. დეტალური ინსტრუქციები შესაბამის გვერდებზეა მოცემული.",
        )
        self.assertEqual(component_payload["data"]["contentData"]["listcount"], 10)
        self.assertEqual(
            [item["title"] for item in items],
            [
                "მოვაჭრის მონაცემები და პირობების მოქმედება",
                "საიტის გამოყენება, ანგარიში და სტუმრის შეკვეთა",
                "პროდუქტის ინფორმაცია და თავსებადობა",
                "ფასი, მარაგი და ტექნიკური შეცდომები",
                "შეკვეთის განთავსება და დადასტურება",
                "გადახდის პირობები",
                "მიწოდების მოკლე პირობები",
                "გაუქმება, დაბრუნება და დამონტაჟებული ნაწილი",
                "იურიდიული პირები და B2B შეკვეთები",
                "კონფიდენციალურობა, უსაფრთხოება და ცვლილებები",
            ],
        )
